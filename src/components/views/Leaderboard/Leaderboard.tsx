...
const podiumConfig: Record<
  number,
  { height: string; bg: string; labelColor: string; order: number }
> = {
...
interface RankRowProps {
  entry: import("./useLeaderboard").RankEntry;
  idx: number;
  showBorder: boolean;
}
...
const RankRow = ({ entry, idx, showBorder }: RankRowProps) => (
  ...
  >
    ...
    <div className="flex items-center gap-3 min-w-0">
      <Avatar className="w-9 h-9 shrink-0 ring-2 ring-white shadow-sm">
        <AvatarFallback
          className={cn(
            entry.isCurrentUser
    ...
    <div className="justify-self-end flex items-center gap-1 bg-primary text-white px-3 py-1 rounded-full shadow-sm">
      ...
      <AppLayout>
        <div className="flex flex-col items-center justify-center py-20 gap-3 text-center">
          ...
          <p className="text-sm text-muted-foreground">
            Selesaikan kuis untuk masuk papan peringkat!
          </p>
        </div>
      </AppLayout>
    );
            ...
            {top3.map((entry) => {
              ...
              return (
                ...
                >
                  ...
                  <Avatar
                    className={cn(avatarSize, "ring-2 ring-white shadow-md")}
                  >
                    <AvatarFallback
                      className={cn(
          ...
          <CardContent className="p-0">
            {rankings.map((entry, idx) => (
              <RankRow
                ...
                entry={entry}
                idx={idx}
                showBorder={idx < rankings.length - 1 || !!currentUserEntry}
              />
            ))}
