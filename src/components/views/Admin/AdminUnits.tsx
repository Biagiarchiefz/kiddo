      ...
      <DialogContent className="max-w-5xl h-[85vh] flex flex-col">
        ...
        <DialogFooter className="shrink-0">
          <p className="text-xs text-muted-foreground mr-auto">
            Gunakan # untuk H1, ## untuk H2, **tebal**, *miring*, dll.
          </p>
          <Button variant="outline" onClick={onClose} disabled={saving}>
            Batal
          </Button>
          <Button
            onClick={() => unit && onSave(unit.id, text)}
            disabled={saving}
          >
  ...
  const [form, setForm] = useState<UnitForm>(
    initial
      ? {
          ...
          unlock_required_correct: initial.unlock_required_correct,
        }
      : emptyUnitForm(),
  );
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="space-y-1">
              ...
              <Input
                type="number"
                min={0}
                value={form.unlock_required_correct}
                onChange={(e) =>
                  set("unlock_required_correct", Number(e.target.value))
                }
              />
            </div>
          </div>
...
const AdminUnits = () => {
  ...
  const { data: mod } = useQuery({
    queryKey: ["admin-module", mid],
    queryFn: () => fetchModule(mid),
    staleTime: 60_000,
  });
  const { data: units = [], isLoading } = useQuery({
    queryKey: ["admin-units", mid],
    queryFn: () => fetchUnits(mid),
    staleTime: 30_000,
  });
  ...
  const upsert = useMutation({
    ...
    onSuccess: (_, { id }) => {
      ...
      toast.success(
        id ? "Unit berhasil diperbarui." : "Unit berhasil ditambahkan.",
      );
      setDialogOpen(false);
      setEditTarget(null);
    },
  ...
  return (
    <AdminLayout>
      <div className="space-y-5">
        ...
        <Card className="border border-border overflow-hidden py-0 gap-0 min-w-[480px]">
          ...
          <CardContent className="p-0">
            {isLoading
              ...
              : units.map((u, i) => (
                  <div
                    key={u.id}
                    className={`flex items-center gap-3 px-5 py-3.5 ${i < units.length - 1 ? "border-b border-border" : ""} hover:bg-muted/20 transition-colors`}
                  >
                    ...
                    <span className="w-20 text-center text-xs text-muted-foreground">
                      {u.unlock_required_correct} soal
                    </span>
                    <div className="w-36 flex items-center justify-center gap-1">
                      <Button
                        ...
                        variant="ghost"
                        size="sm"
                        className="h-7 px-2 text-xs gap-1 text-sky-600 hover:text-sky-700 hover:bg-sky-50"
                      >
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              Hapus unit "{deleteTarget?.title}"?
            </AlertDialogTitle>
            <AlertDialogDescription>
              Semua soal di unit ini juga akan terhapus.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
