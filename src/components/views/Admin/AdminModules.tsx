...
const AdminModules = () => {
  ...
  const upsert = useMutation({
    ...
    onSuccess: (_, { id }) => {
      ...
      toast.success(
        id ? "Modul berhasil diperbarui." : "Modul berhasil ditambahkan.",
      );
      setDialogOpen(false);
      setEditTarget(null);
    },
    ...
    <AdminLayout>
          ...
          <CardContent className="p-0">
            {isLoading
              ...
              : paginated.map((m, i) => (
                  <div
                    key={m.id}
                    className={`flex items-center gap-3 px-5 py-3.5 ${i < paginated.length - 1 ? "border-b border-border" : ""} hover:bg-muted/20 transition-colors`}
                  >
                    ...
                    <div className="w-14 shrink-0">
                      {m.image_url ? (
                        ...
                        <div
                          className={`w-10 h-10 rounded-lg flex items-center justify-center text-xl ${m.header_bg}`}
                        >
                          {m.emoji || "📚"}
                        </div>
                      )}
                    ...
                    <span
                      className={`w-20 text-center text-[10px] font-bold px-2 py-0.5 rounded-full ${m.is_published ? "bg-green-100 text-green-700" : "bg-muted text-muted-foreground"}`}
                    >
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              Hapus modul "{deleteTarget?.title}"?
            </AlertDialogTitle>
            <AlertDialogDescription>
              Semua unit dan soal di dalam modul ini juga akan terhapus.
